...
impl TempNode {
    fn sort_children_recursive(&mut self, key: &SortKey, reverse: bool) {
        if !self.children.is_empty() {
            self.children
                .sort_by(|a, b| compare_siblings(a, b, key, reverse));
...
fn build_tree(nodes_info: Vec<NodeInfo>) -> Vec<TempNode> {
    ...
    for node_info in nodes_info {
        ...
        while !path_stack.is_empty() {
            let parent_depth = get_node_by_path(&roots, &path_stack).unwrap().node_info.depth;
        ...
        if path_stack.is_empty() {
            ...
            let parent = get_node_mut_by_path(&mut roots, &path_stack).unwrap();
...
fn flatten_tree_to_dfs_consuming(roots: Vec<TempNode>, result: &mut Vec<NodeInfo>) {
...
fn compare_siblings(a: &TempNode, b: &TempNode, key: &SortKey, reverse: bool) -> Ordering {
    let ord = match key {
        ...
        SortKey::Lines => match (a.node_info.line_count, b.node_info.line_count) {
